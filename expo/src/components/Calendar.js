...
const Calendar = ({ onScrollToDate, selectedMonth }) => {
  ...
  const { dailyDoses, calendarDays, calendarGoalsStartOfWeek, abstinenceDays } = useRecoilValue(
  ...
  const calendarDayByWeek = useMemo(() => {
    ...
    return daysByWeek;
  }, [nbDays, firstDayOfCalendar, calendarDays, today, calendarGoalsStartOfWeek, goals]);
