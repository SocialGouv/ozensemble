  ...
  switch (period) {
    case "daily":
      return firstDay.get("month") === lastDay.get("month")
        ? `Semaine du ${dayjs(firstDay).format("D")} au ${dayjs(lastDay).format("D")} ${dayjs(
            lastDay
          ).format("MMMM")}`
        : `Semaine du ${dayjs(firstDay).format("D")} ${dayjs(firstDay).format("MMM")} au ${dayjs(
            lastDay
          ).format("D")} ${dayjs(lastDay).format("MMM")}`;
    case "weekly":
      return firstDay.get("month") === lastDay.get("month")
        ? `6 semaines du ${dayjs(firstDay).format("D")} au ${dayjs(lastDay).format("D")} ${dayjs(
            lastDay
          ).format("MMMM")}`
        : `6 semaines du ${dayjs(firstDay).format("D")} ${dayjs(firstDay).format("MMM")} au ${dayjs(
            lastDay
          ).format("D")} ${dayjs(lastDay).format("MMM")}`;
    case "monthly":
      return firstDay.get("year") === lastDay.get("year")
        ? `6 mois de ${dayjs(firstDay).format("MMMM")} à ${dayjs(lastDay).format("MMMM")} ${dayjs(
            lastDay
          ).format("YYYY")}`
        : `6 mois de ${dayjs(firstDay).format("MMM")} ${dayjs(firstDay).format("YYYY")} à ${dayjs(
            lastDay
          ).format("MMM")} ${dayjs(lastDay).format("YYYY")}`;
    default:
      return "";
  }
};
