...
const AbstinenceSelection = ({ navigation, route }) => {
  const isFromInfo = route?.params ? route?.params?.parent === "Infos" : false;
  const onClose = () => {
    navigation.goBack();
  };
  return (
    <SafeAreaView className="flex flex-grow justify-center items-center">
      <View className="bg-white rounded-xl max-w-[90%]">
        <View className=" flex w-full mb-2 px-2">
          <Text className="text-center mb-3 text-xl mt-8 px-2 font-extrabold">
            Compteur d'abstinence
          </Text>
          {!!isFromInfo && (
            <Text className="line text-lg text-justify  color-black mb-5 mx-6 mt-2">
              Il sera affiché sous votre calendrier et vous permettra de suivre vos jours
              consécutifs sans consommation d'alcool si vous avez fait le choix d'être abstinent.
            </Text>
          )}
          {!isFromInfo && (
            <Text className="line text-lg text-justify  color-black mb-5 mx-6 mt-2">
              Il vous permettra de suivre vos jours consécutifs sans consommation d’alcool si vous
              avez fait le choix d’être abstinent. Vous pourrez toujours modifier ce choix dans
              l’onglet Infos.
            </Text>
          )}
