...
const currentBuildNumber = require("./app.versions.json").buildNumber; // ex: 2.1.1
const currentBuildName = require("./app.versions.json").buildName; // ex: 127

const newBuildNumber = currentBuildNumber + 1;
const newBuildName = release !== "bump" ? semver.inc(currentBuildName, release) : currentBuildName;
...
// Replace the version in the ios/ozensemble.xcodeproj/project.pbxprojfile via regex and save it
const projectPbxproj = fs.readFileSync("ios/ozensemble.xcodeproj/project.pbxproj", "utf8");
const newProjectPbxproj = projectPbxproj
  .replaceAll(/CURRENT_PROJECT_VERSION = [^;]+;/gi, `CURRENT_PROJECT_VERSION = ${newBuildNumber};`)
  .replaceAll(/MARKETING_VERSION = [^;]+;/gi, `MARKETING_VERSION = ${newBuildName};`);
fs.writeFileSync("ios/ozensemble.xcodeproj/project.pbxproj", newProjectPbxproj);
...
// Replace the versions in the app.versions.json file via regex and save it
const appJson = fs.readFileSync("app.versions.json", "utf8");
const newAppJson = appJson
  .replace(/"buildName": "[^"]+"/, `"buildName": "${newBuildName}"`)
  .replace(/"buildNumber": [^"]+,/, `"buildNumber": ${newBuildNumber},`);
fs.writeFileSync("app.versions.json", newAppJson);
