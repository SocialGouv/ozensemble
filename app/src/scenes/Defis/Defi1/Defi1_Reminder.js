...
const Defi1_Reminder = ({ navigation, route }) => {
  ...
  const activateNotif = async () => {
    ...
    if (!NotificationService.hasToken()) navigation.navigate(...route.params.onPressContinueNavigation); // if already asked, don't block the user, go to next screen. onRegister will be triggered on next launch ?
